...
class YamlFilterTests {
    ...
    private final String yamlFilePath = "src/main/resources/member-system-openapi.yaml";
    ...
    System.out.println(resultYaml);
        ...
        // Assert
        assertNotNull(filteredData, "Filtered data should not be null for a valid API path and method.");
        assertFalse(resultYaml.isEmpty(), "Resulting YAML should not be empty.");

        // 驗證只保留了指定的 API
        assertTrue(resultYaml.contains("  /user/profile:"), "The YAML should contain the specified API path.");
        assertTrue(resultYaml.contains("    get:"), "The YAML should contain the specified HTTP method.");
        ...
        // 驗證同一路徑下的其他方法已被移除
        assertFalse(resultYaml.contains("    put:"), "Other HTTP methods under the same path should be removed.");
        assertFalse(resultYaml.contains("    delete:"), "Other HTTP methods under the same path should be removed.");
    ...
    void testFilterByApi_MethodNotFound() {
        ...
        // Assert
        assertNull(filteredData, "Should return null when the HTTP method is not found for the given path.");
    }
